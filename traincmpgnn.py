import argparse
from Baselinenets.dataset_loader import DataLoader
from Baselinenets.utils import random_planetoid_splits
import torch
...
class StepConv(MessagePassing):
    ...
    def forward(self, h1):
        ...
        if self.edgeweight==1:
            ...
            h4 = self.propagate(self.edge, size=(numnode, numnode), x=h4, norm=1-2*s, flow='source_to_target')
            x = torch.sub(h3, self.step1 * h4)

        if self.step2 == 0:
            C = x
        else:
            edge_index1, edge_index2, o, e = self.split_edge_index(x)
            if self.dprate==0:
                ho = self.propagate(edge_index1, size=(numnode, numnode), x=x, norm=o, flow='source_to_target')
                he = self.propagate(edge_index2, size=(numnode, numnode), x=x, norm=e, flow='source_to_target')
                x = torch.add(torch.add(x, self.step2 * ho), torch.sub(x, self.step2 * he))

            else:
                ho = self.propagate(edge_index1, size=(numnode, numnode), x=x, norm=o, flow='source_to_target')
                he = self.propagate(edge_index2, size=(numnode, numnode), x=x, norm=e, flow='source_to_target')
                ho = F.dropout(ho, p=self.dprate, training=self.training)
                he = F.dropout(he, p=self.dprate, training=self.training)
                x = torch.add(torch.add(x, self.step2 * ho), torch.sub(x, self.step2 * he))
            C = x
            #C = self.classify(x)
        return C

    def split_edge_index(self, x):

        tt = torch.mul(x[self.row], x[self.col])
        st = torch.sigmoid(torch.sum(tt, dim=1))
        #  spilit the edge into edge_index1 and dge_index2  with the aid of gate function
        mask = (st > 0.5)
        edge_index1 = self.edge_index[:, mask]
        edge_index2 = self.edge_index[:, ~mask]
        # print(edge_index1.shape[1])
        # print(edge_index2.shape[1])
        # num=edge_index1.shape[1]+edge_index2.shape[1]
        # print(num)

        o = (1 - st)[mask]
        e = st[~mask]

        return edge_index1, edge_index2, o, e
...
class CMPGNN(nn.Module):
    def __init__(self, dataset, data,args):
        ...
        self.layers1 = nn.ModuleList([StepConv(self.num_hidden, self.num_hidden, data.edge_index,args) for _ in range(args.K)])
...
def RunExp(args, dataset, data,  Net, percls_trn, val_lb):
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'savemodels/{}_{}_weights_{}.pth'.format(args.net, args.dataset, args.train_rate))
        if epoch >= 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--activation', type=str, choices=['relu', 'elu', 'None'],
                        default='None')
    parser.add_argument('--edge', type=int, default=1, help='edge weight.')
    parser.add_argument('--Norm', type=int, default=1, help='Norm.')
    parser.add_argument('--step1', type=float, default=2.0, help='for CMPGNN.')
    parser.add_argument('--step2', type=float, default=0.0, help='for CMPGNN.')
    ...
    for RP in tqdm(range(args.runs)):
        args.seed = SEEDS[RP]
        test_acc, best_val_acc,time_run = RunExp(args, dataset, data,Net,   percls_trn,
                                                           val_lb)
    ...
    filename = f'{args.net}_{args.dataset}.csv'
    print(f"Saving results to {filename}")
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(
             "net:{}, dataset:{},  lr:{}, weight_decay:{}, TrainRate:{}, K:{},   activation:{}, step1:{}, step2:{}, acc_test:{}, accstd:{},".format(
                args.net, args.dataset, args.lr, args.weight_decay, args.train_rate, args.K, args.activation, args.step1, args.step2, test_acc_mean, test_acc_std)) 
        write_obj.write("\n")
